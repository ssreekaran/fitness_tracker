...
import { WorkoutTemplate } from "../services/workoutPlanningService";
...
const WorkoutPlanner: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState("calendar"); // New tab state
  const { user } = useAuth();
  ...
  const handleDeleteSeriesAfter = async () => {
    ...
    try {
      ...
      setEvents((prev) =>
        prev.filter(
          (ev) => ev.seriesId !== selectedEvent.seriesId || ev.start <= cutoff
        )
      );
  ...
  const addInterval = (
    date: Date,
    mode: "daily" | "weekly" | "biweekly" | "monthly" | "custom",
    every: number,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const d = new Date();
      d.setMonth(d.getMonth() + 1);
      setRepeatUntil(toLocalDatetimeString(d).split("T")[0]);
      setShowModal(false);
    } catch (error) {
  ...
  const handleDeleteSeries = async () => {
    ...
    try {
      ...
      setEvents((prev) =>
        prev.filter((ev) => ev.seriesId !== selectedEvent.seriesId)
      );
      setSelectedEvent(null);
    } catch (err) {
  ...
  // Handle template selection from AI planner
  const handleTemplateSelection = (template: WorkoutTemplate) => {
    ...
    setShowModal(true);
    setActiveTab("calendar"); // Switch to calendar tab to show the form
  };
  ...
  const handleDeleteEvent = async () => {
    ...
    try {
      await deleteDoc(
        doc(db, "users", user.uid, "workoutPlans", selectedEvent.id)
      );
  ...
  useEffect(() => {
    const fetchWorkoutPlans = async () => {
      ...
      try {
        ...
        const userWorkouts = querySnapshot.docs
          .map((doc) => {
            ...
            if (
              ...
              console.warn(
                "Skipping workout plan with invalid timestamps:",
                doc.id,
                { start, end }
              );
              skippedCount++;
        ...
        if (skippedCount > 0) {
          console.warn(
            `Skipped ${skippedCount} workout plans due to invalid timestamps`
          );
        }
      ...
      <div className="wp-layout">
        <div className="wp-hero">
          <div className="wp-hero-text">
            <h1 className="wp-title">🏋️ Smart Workout Planner</h1>
            <p className="wp-subtitle">
              AI-powered planning meets calendar scheduling.
            </p>
          </div>
          ...
          <Tabs
            ...
            className="wp-tabs mb-4"
          >
            <Tab eventKey="calendar" title="📅 Calendar Planner">
                ...
                <div className="wp-hero-actions">
                  ...
                  <div className="type-legend inside-hero">
                    <span className="legend-item">
                      <span className="dot strength"></span>Strength
                    </span>
                    <span className="legend-item">
                      <span className="dot cardio"></span>Cardio
                    </span>
                    <span className="legend-item">
                      <span className="dot flexibility"></span>Flexibility
                    </span>
                    <span className="legend-item">
                      <span className="dot rest"></span>Rest
                    </span>
                  </div>
                </div>
                      ...
                      <>
                        <div className="row g-3 align-items-end mb-3">
                          <Form.Group className="col-md-6">
                            ...
                            <Form.Select
                              ...
                              onChange={(e) =>
                                setRepeatMode(
                                  e.target.value as typeof repeatMode
                                )
                              }
                            >
                            ...
                            <>
                              ...
                              <Form.Group className="col-md-3">
                                ...
                                <Form.Select
                                  ...
                                  onChange={(e) =>
                                    setCustomUnit(
                                      e.target.value as typeof customUnit
                                    )
                                  }
                                >
                        ...
                        <Form.Group className="mb-3">
                          ...
                          <Form.Control
                            ...
                            min={
                              toLocalDatetimeString(formData.start).split(
                                "T"
                              )[0]
                            }
                            max={(() => {
                              const m = getMaxUntilDate(formData.start);
                              return toLocalDatetimeString(m).split("T")[0];
                            })()}
                          />
                          <div className="d-flex flex-wrap gap-2 mt-2">
                            <Button
                              ...
                              onClick={() => {
                                ...
                                d.setMonth(d.getMonth() + 1);
                                setRepeatUntil(
                                  toLocalDatetimeString(d).split("T")[0]
                                );
                              }}
                            >
                              +1 mo
                            ...
                            <Button
                              ...
                              variant="outline-secondary"
                              onClick={() => {
                                const d = getMaxUntilDate(formData.start);
                                setRepeatUntil(
                                  toLocalDatetimeString(d).split("T")[0]
                                );
                              }}
                          ...
                          <Form.Text muted>
                            Recurrence is capped at 5 years to avoid clutter.
                          </Form.Text>
                        </Form.Group>
                      </>
                    )}
              ...
              >
                ...
                <Modal.Body>
                  <p>
                    This will permanently delete all workouts in your planner.
                    This action cannot be undone. If you want to keep past data,
                    consider deleting a series or deleting occurrences after a
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const refsToDelete: DocumentReference[] = snap.docs.map(
                          (d) => d.ref
                        );
                        await commitInChunks(db, refsToDelete, 500);
                        setEvents([]);
            ...
            <Tab eventKey="ai-planner" title="🧠 AI Planner">
              <div className="ai-planner-tab-content">
                <IntelligentWorkoutPlanner
                  userId={user?.uid || ""}
                  userWeight={70} // You can get this from user profile
                  onWorkoutSelected={handleTemplateSelection}
                />
              </div>
            </Tab>
          </Tabs>
