...
async function loadCSVFile<T>(path: string): Promise<T[]> {
  try {
    ...
    if (!isValidCSV) {
      throw new Error(
        `Unexpected content-type '${contentType}' when loading ${path}. Expected a CSV file.`
      );
    }
...
const loadFoodItems = async (): Promise<void> => {
  try {
    ...
    const foodNameData = await loadCSVFile<FoodNameEntry>(
      "/cnf-fcen-csv/FOOD NAME.csv"
    );
    ...
    const findColumn = (
      ...
      if (!key) {
        console.warn(
          `Could not find any of these columns: ${possibleNames.join(", ")}`
        );
    ...
    const foodIdKey = findColumn(
      ["FoodID", "FoodCode", "food_id", "foodid", "foodId"],
      availableKeys
    );
    const foodDescKey = findColumn(
      [
        "FoodDescription",
        "Description",
        "food_desc",
    ...
    if (!foodIdKey || !foodDescKey) {
      throw new Error(
        `Missing required columns. Found: ${availableKeys.join(", ")}`
      );
    }

    console.log(
      `Using column names: '${foodIdKey}' for ID, '${foodDescKey}' for description`
    );
    ...
    const nutrientData = await loadCSVFile<NutrientEntry>(
      "/cnf-fcen-csv/NUTRIENT AMOUNT.csv"
    );
    ...
    const nutrientIdKey = findColumn(
      ["NutrientID", "nutrient_id", "nutrientid"],
      availableNutrientKeys
    );
    const nutrientValueKey = findColumn(
      ["NutrientValue", "Value", "nutrient_value"],
      availableNutrientKeys
    );
    const nutrientFoodIdKey = findColumn(
      ["FoodID", "food_id", "foodid", "FoodCode"],
      availableNutrientKeys
    );

    console.log("Nutrient columns:", {
      nutrientIdKey,
      nutrientValueKey,
      nutrientFoodIdKey,
    });
    ...
    const missingColumns = requiredColumns.filter(
      (col) => !availableNutrientKeys.includes(col)
    );
    ...
    if (missingColumns.length > 0) {
      throw new Error(
        `Missing required columns: ${missingColumns.join(
          ", "
        )}. Available columns: ${availableNutrientKeys.join(", ")}`
      );
    }
    ...
    if (!nutrientIdKey || !nutrientValueKey) {
      throw new Error(
        `Missing required nutrient columns. Found: ${availableNutrientKeys.join(
          ", "
        )}`
      );
    }
    ...
    console.log(
      `Using column names: '${nutrientIdKey}' for nutrient ID, '${nutrientValueKey}' for value`
    );
    ...
    const possibleNutrientNameKeys = [
      "NutrientName",
      "Nutrient_Name",
      "NUTRIENTNAME",
      "nutrientname",
    ...
    if (!nutrientNameKey) {
      console.warn(
        "Could not determine nutrient name column, falling back to default"
      );
    ...
    for (const nutrient of nutrientData) {
      try {
        ...
        const id = String(
          nutrient[nutrientIdKey as keyof NutrientEntry] ?? ""
        ).trim();
        if (!id) continue;

        // Safely get the name using the detected key with fallback
        const name = String(
          nutrient[nutrientNameKey as keyof NutrientEntry] ?? ""
        )
          .trim()
          .toLowerCase();
        if (!name) continue;
    ...
    const findBestNutrientId = (config: {
      id: string;
      name: string;
    }): string => {
      ...
      if (nutrientNameMap.has(nameKey)) {
        ...
        if (ids.length > 0) {
          console.warn(
            `Using alternative ID ${ids[0]} for ${config.name} (original: ${config.id})`
          );
          return ids[0];
        }
      }
      ...
      for (const [name, ids] of nutrientNameMap.entries()) {
        if (name.includes(nameKey) || nameKey.includes(name)) {
          ...
          console.warn(
            `Using partial match ${idArray[0]} for ${config.name} (original: ${config.id})`
          );
          return idArray[0];
        }
      }
      ...
      console.warn(
        `No match found for nutrient: ${config.name} (ID: ${config.id})`
      );
      return "";
    };
    ...
    for (const nutrient of nutrientData) {
      try {
        const foodId = String(
          nutrient[nutrientFoodIdKey as keyof NutrientEntry] || ""
        )
          .trim()
          .toLowerCase();
        if (!foodId) continue;
    ...
    for (let i = 0; i < foodNameData.length; i++) {
      ...
      if (!foodDesc) {
        console.warn(
          `Skipping food entry with ID ${foodId} missing description`
        );
        continue;
      }
      ...
      if (validItems < 3) {
        console.log("Processing food item:", {
          foodId,
          foodDesc,
          normalizedCode,
        });
      ...
      if (i < 3 || i % 1000 === 0) {
        console.log(
          `Processing food item ${i + 1}/${
            foodNameData.length
          }: ID ${foodId}, Nutrients found:`,
          nutrients.length
        );
      }
      ...
      const getNutrientValue = (
        type: keyof typeof DEFAULT_NUTRIENT_MAPPING
      ): number => {
        try {
          ...
          if (i < 3) {
            console.log(
              `Looking for ${type} (ID: ${targetNutrientId}) in food ${foodId}...`
            );
          }
          ...
          const nutrient = nutrients.find((n: Record<string, unknown>) => {
            try {
              const currentId = String(
                n[nutrientIdKey as keyof NutrientEntry] || ""
              ).trim();
          ...
          if (!nutrient) {
            if (i < 3) {
              ...
              console.warn(
                `Nutrient ${type} (${config.name}, ID: ${targetNutrientId}) not found for food ${foodId}`
              );
              console.log(
                "Available nutrient IDs:",
                nutrients.map((n: Record<string, unknown>) => ({
          ...
          if (isNaN(value)) {
            console.warn(
              `Invalid numeric value for ${type}: '${valueStr}' (raw: '${rawValue}')`
            );
            return 0;
          }

          if (i < 3) {
            // Log first few successful lookups
            console.log(
              `Found ${type}: ${value} ${config.unit} for food ${foodId}`
            );
          }
...
export const initFoodDatabase = async (): Promise<void> => {
  ...
  if (initializationAttempts >= MAX_INIT_ATTEMPTS) {
    ...
    if (timeSinceLastAttempt < retryAfterMs) {
      const timeUntilRetry = Math.ceil(
        (retryAfterMs - timeSinceLastAttempt) / 1000
      );
      throw new Error(
  ...
  initializationPromise = (async () => {
    try {
      console.log(
        `Initializing food database (attempt ${initializationAttempts}/${MAX_INIT_ATTEMPTS})...`
      );
      ...
      console.error(
        `Food database initialization failed (attempt ${initializationAttempts}/${MAX_INIT_ATTEMPTS}):`,
        error
      );
